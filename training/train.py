...
def train_one_epoch(model: nn.Module,
                   ...
                   use_ctc: bool = False) -> Tuple[float, float]:
    """
    Train for one epoch.
...
def train_model(config: Dict, device: str = None, resume_from: str = None):
    ...
    val_dataset = SignLanguageDataset(
        config['data']['val_csv'],
        Path(config['data']['val_csv']).parent / 'features'
    )
    ...
    for epoch in range(start_epoch, config['training']['epochs']):
        ...
        if (epoch + 1) % config['training']['val_every'] == 0:
            val_loss, val_acc = validate(model, val_loader, criterion, device, use_ctc=use_ctc)
...
def main():
    parser = argparse.ArgumentParser(description="Train BiLSTM model for sign language recognition")
