...
export default function ExploreView({ glosses, summary, selected, onSelect }: Props) {
  ...
  useEffect(() => {
    if (!activeClip?.has_landmarks || activeClip.has_video) return;
    let frame = 0;
    const fps = activeClip.fps > 0 ? activeClip.fps : 25;
    const tick = () => {
      const skel = skelRef.current;
      if (!skel) return;
      frame = (frame + 1) % Math.max(1, activeClip.frame_count);
      skel.src = `${include50SkeletonUrl(activeClip.word, activeClip.stem, frame)}?t=${frame}`;
    };
    tick();
