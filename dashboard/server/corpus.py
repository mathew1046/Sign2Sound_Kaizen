...
_MISSING_VIDEO = Path("__missing_video__")


def _is_video_file(path: Path) -> bool:
    return path.suffix.lower() in _VIDEO_SUFFIXES and path.is_file()
...
class Include50Corpus:
    ...
    def _resolve_vol_input_path(self, manifest_path: str) -> Path | None:
        """Map Modal ``/vol/input/...`` or vocab ``include-50/...`` paths to the local tree.

        Local layout::
            include-50/{Category}_{NofM}/{Category}/{label folder}/MVI_xxxx.MOV
        e.g. ``Adjectives_1of8/Adjectives/1. loud/MVI_5258.MOV``
        """
        p = Path(manifest_path)
        rel: str | None = None
        if manifest_path.startswith("/vol/input/"):
            rel = manifest_path[len("/vol/input/") :].lstrip("/")
        elif manifest_path.startswith("include-50/"):
            rel = manifest_path[len("include-50/") :].lstrip("/")

        if not rel:
            return None

        stem = p.stem
        candidates: list[Path] = [self.video_root / rel]
        parts = rel.split("/", 1)
        if len(parts) == 2:
            category, rest = parts
    ...
    def _video_from_meta(self, word: str, stem: str) -> Path | None:
        ...
        try:
            ...
            return None
        video_ref = str(meta.get("video", "")).strip()
        if not video_ref:
            return None
        vol_hit = self._resolve_vol_input_path(video_ref)
        if vol_hit is not None and vol_hit.is_file():
            return vol_hit
        resolved = C.resolve_video_path(video_ref)
        if resolved.is_file():
            return resolved
    ...
    def _resolve_video_path(self, word: str, stem: str, manifest_path: str) -> Path:
        meta_hit = self._video_from_meta(word, stem)
        ...
        if path.suffix.lower() in _VIDEO_SUFFIXES:
            resolved = C.resolve_video_path(manifest_path)
            if resolved.is_file():
                return resolved
        ...
        hit = self._video_by_word_stem.get((word, stem))
    ...
    def corpus_summary(self) -> dict:
        ...
        with_video = sum(1 for c in clips if _is_video_file(c.source_path))
        with_skel = sum(
    ...
    def corpus_glosses(self) -> list[dict]:
        from dashboard.config import load_corpus_vocabulary
    ...
    def playable_video(self, word: str, stem: str) -> Path:
        ...
        if clip is None:
            raise KeyError(f"Unknown clip: {word}/{stem}")
        src = clip.source_path
        if not _is_video_file(src):
            raise FileNotFoundError(
                f"RGB video missing for {word}/{stem}. "
                f"Skeleton playback is still available. "
                f"Optional videos: place INCLUDE-50 under {self.video_root}"
            )
        if src.suffix.lower() == ".mp4" and src.is_file():
            from dashboard.server.collection.transcode import is_browser_playable

            if is_browser_playable(src):
                return src
        from dashboard.server.collection.transcode import ensure_mp4
